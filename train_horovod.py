...
"""
mpirun --mca btl_vader_single_copy_mechanism none --allow-run-as-root -bind-to none -map-by slot -mca orte_base_help_aggregate 0 -x NCCL_DEBUG=INFO -np 2 -H localhost:2 python train_horovod.py --input_file_pattern={processed_data_path}/train-?????-of-00256 --inception_checkpoint_file={inception_v3_file} --train_inception=false --number_of_steps=200
...
if hvd.rank() == 0:
    ...
    ckpt_hook = tf.train.CheckpointSaverHook('horovod_ckpt', save_steps=1, scaffold=scaffold)
    ...
    for i in range(FLAGS.number_of_steps):
        if not sess.should_stop():
            ...
            if i % FLAGS.log_every_n_steps  == 0:
                ...
                print("step: %d, loss: %lf, throuphput: %f steps/sec" % (i, loss_, throughput))
